...
const ConnectconIndex: FCWithLayoutParams<never> = () => {
  const { t } = useTranslation('connectcon/1/home')
  ...
  // @ts-ignore
  const speakers = [...Array(6).keys()].map((index) => {
    return {
  ...
  const scenes = ['first', 'second', 'third'].map((stage) => {
    return {
      ...
      subtitle:
        t(`sections.seventh.scenes.${stage}.subtitle`) !== `connectcon/1/home:sections.seventh.scenes.${stage}.subtitle`
      ...
      rows: [...Array(6).keys()]
        .map((index) => {
          if (
            ...
            `connectcon/1/home:sections.seventh.scenes.${stage}.rows.${index + 1}`
          ) {
    ...
    <div className={clsx(styles.landing, 'container')}>
      ...
      <div className={styles.landing__section}>
        <h1 className={styles.landing__h1}>
          {t('sections.first.heading')} <br />
          <div dangerouslySetInnerHTML={{ __html: t('sections.first.date') }} />
        </h1>
        <div dangerouslySetInnerHTML={{ __html: t('sections.first.content') }} />
      </div>
      ...
      <div className={styles.landing__section}>
        ...
        {/*<p className={styles.landing__highlight} dangerouslySetInnerHTML={{ __html: t('sections.second.content_highlight') }} />*/}
        <div dangerouslySetInnerHTML={{ __html: t('sections.second.content_2') }} />
      </div>

      {/* Section 3 */}
      <div className={styles.landing__section}>
        <div className={styles.landing__h2}>{t('sections.third.heading')}</div>
      ...
      <div className={clsx(styles.landing__section)}>
        ...
        {/*    <a href="https://connectclub.notion.site/ConnectCon-Speakers-in-progress-93a722e54bd74a07bbccd28d8150d1f8" rel="noreferrer" target='_blank' title={t('sections.fourth.button_name')} className={styles.landing__button}>*/}
